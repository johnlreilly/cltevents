            ...
            <>
              ...
              <div className="absolute top-full left-0 mt-2 w-full bg-surface rounded-2xl shadow-2xl z-50 max-h-96 overflow-y-auto border border-outlinevariant">
                ...
                {availableGenres.map((genre) => {
                  // When no genres are selected, all are shown (checked)
                  // When genres are selected, only selected ones are shown (checked)
                  const isChecked = selectedGenres.length === 0 || selectedGenres.includes(genre)
                  return (
                    <button
                      key={genre}
          ...
          {showSourceMenu && (
            <>
              ...
              <div className="absolute top-full left-0 mt-2 w-full bg-surface rounded-2xl shadow-2xl z-50 max-h-96 overflow-y-auto border border-outlinevariant">
                ...
                {sources.map((source) => {
                  // When no sources are selected, all are shown (checked)
                  // When sources are selected, only selected ones are shown (checked)
                  const isChecked = selectedSources.length === 0 || selectedSources.includes(source)
                  return (
                    <button
                      key={source}
