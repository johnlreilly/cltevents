 ...
 * Prioritizes faces and important features using smartcrop's built-in algorithms
...
export const getSmartCrop = async (imageUrl, width, height) => {
  try {
    ...
    // Configure smartcrop with optimal settings for face detection
    const options = {
      width,
      height,
      minScale: 1.0,
      ruleOfThirds: true,
      // Smartcrop has built-in face detection via skin tone analysis
      // These weights help it prioritize faces
      ...
      // Step size - lower is more accurate but slower
      scoreDownSample: 8,
      step: 8,
    }
